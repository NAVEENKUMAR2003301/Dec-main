...
const validateContactRequest = (payload) => {
    ...
    if (!record.name || !record.email || !record.phone || !record.message) {
        throw createValidationError("Name, email, phone, and message are required.");
    }
...
const validateConsultation = (payload) => {
    ...
    if (!record.selectedTopics.length) {
        throw createValidationError("Please choose at least one consultation topic.");
    }
...
const insertConsultation = (record, options = {}) => {
    ...
    try {
        const result = statement.run(
            ...
            record.selectedTeamMember ? JSON.stringify(record.selectedTeamMember) : null,
            record.eventDate || null,
            record.guestCount,
            record.venue || null,
            record.message || null,
        ...
        if (
            ...
            error.message.includes("UNIQUE constraint failed: consultations.selected_date, consultations.selected_time")
        ) {
...
const getDatabaseCounts = () => ({
    contactRequests: database.prepare("SELECT COUNT(*) AS count FROM contact_requests").get().count,
    consultations: database.prepare("SELECT COUNT(*) AS count FROM consultations").get().count,
});
...
const migrateLegacyJsonData = () => {
    ...
    try {
        ...
        console.warn("Legacy JSON migration was skipped because the file could not be read.");
        console.warn(error);
    }
};
...
const startServer = async () => {
    const server = createServer(async (request, response) => {
        ...
        try {
            ...
            if (request.method === "GET" && url.pathname === "/api/contact-requests") {
                sendJson(response, 200, {
            ...
            if (request.method === "POST" && url.pathname === "/api/contact-requests") {
                ...
                sendJson(response, 201, {
                    message: "Thank you for reaching out! We've saved your message and will get back to you within 24 hours.",
                    item: record,
                });
                return;
            }
            ...
            if (request.method === "POST" && url.pathname === "/api/consultations") {
                ...
                sendJson(response, 201, {
                    message: "Your consultation has been booked successfully! We've saved your details and reserved the selected slot.",
                    item: record,
                });
                return;
            }
            ...
            if (typeof error.message === "string" && error.message.toLowerCase().includes("database is locked")) {
    ...
    server.on("error", (error) => {
        if (error.code === "EADDRINUSE") {
            console.error(`Port ${port} is already in use. Stop the old backend process and run the command again.`);
            process.exit(1);
            return;
        }
